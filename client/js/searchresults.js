...
//Student 220030280's and 220014528's  code 
...
function applyFilters() {
  ...
  // clear markers shown before
  for(let i = 0; i < Object.keys(markers).length; i ++){
    let markerId = Object.keys(markers)[i];
    ...
    if(
      (filters.location && filteredObjects[i].location != filters.location) ||
        ...
        // continue;
          ...
          // mark filtered results on the map 
          filteredResultsName = filteredObjects[i].activityName;
          filteredResultLat = filteredObjects[i].lat;;
          filteredResultLong = filteredObjects[i].long;;
          filteredResultsLatLng = new google.maps.LatLng(filteredResultLat, filteredResultLong);
          console.log("filteredObjects[i] is ", filteredObjects[i])
          map.panTo(filteredResultsLatLng);
          placeMarkerAndPanTo(filteredResultsLatLng, map, filteredResultsName, filteredResultsName)
...
function initMap(){
  ...
  const urlLONG = parseFloat(params.get('LONG'));
  console.log("url is ", urlLAT, urlLONG)
  map = new google.maps.Map(document.getElementById("map"),{
        center: {lat: urlLAT, lng:urlLONG},
        zoom: 14,
        })
    // get the lat and long from the database
    fetch('/activity/get', {
...
function placeMarkerAndPanTo(latLng, map, activityName, contentString) {
    ...
    console.log("markerId is", marker.id)
