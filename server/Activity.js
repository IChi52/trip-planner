...
//get all activities that are 5 star rated

router.get('/get/5star', function (req, res) {
    collection.find({"reviews.rating": "5"}).toArray() 
        .then(doc => {
            res.status(200).json(doc)
            console.log('find all')
            console.log(doc)
...
router.get('/get/:id', function (req, res) {
    ...
    collection.findOne({_id:mongoId})
        .then(doc => {
            if (doc) {
                res.status(200).json(doc)
                console.log(`find actiity with id ${mongoId}`)
                console.log(doc)
            }else{
                res.status(400).json({msg:`Could not find activity with id ${mongoId}`})
            }
            
        })
        .catch(err => {
            console.log(`Error finding activity with id ${mongoId}: ${err}`)
            res.status(400).json({ msg: `Error finding activity with id ${mongoId}` })
        })

})

//return activity based on lat and long
router.get('/get/activity/:LAT/:LONG', function (req, res) {
    ...
    collection.find({lat: lat, long: long}).toArray()
        .then(doc => {
            if (doc) {
                res.status(200).json(doc)
                console.log(`found actity with LAT ${LAT}`)
                console.log(doc)
            }else{
                res.status(400).json({msg:`Could not find activity with LAT ${LAT}`})
            }
            
        })
        .catch(err => {
            console.log(`Error finding activity with LAT ${LAT}: ${err}`)
            res.status(400).json({ msg: `Error finding activity with LAT ${LAT}` })
        })
